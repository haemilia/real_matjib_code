...
class TextPreprocessor(torch.nn.Module):
    ...
    def __init__(self, config, device):
        ...
        else: # Default: Just concat + project
            self.proj_default_final = nn.Linear(5 * self.fusion_embed_dim, self.fusion_embed_dim)
    ...
    def _tokenize_and_chunk_long_text(self, texts: list[str], max_length: int, chunk_overlap: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for text in texts:
            ...
            tokenized = self.tokenizer(
                ...
                return_tensors='pt' # This creates CPU tensors by default
            )

            # IMMEDIATELY move to device after tokenizer creates them
            input_ids = tokenized['input_ids'].squeeze(0).to(self.device, dtype=torch.long)
            attention_mask = tokenized['attention_mask'].squeeze(0).to(self.device, dtype=torch.long)

            if input_ids.size(0) <= max_length:
                # Use torch.full_like for padding to create tensors
                ...
                batch_input_ids.append(padded_input_ids.unsqueeze(0)) # .unsqueeze(0) creates new tensor
                ...
                for i in range(0, input_ids.size(0), stride):
                    ...
                    if chunk_input_ids.size(0) < max_length:
                        # Again, padding should create on device
                    ...
                    chunks_input_ids.append(chunk_input_ids) # These are already on device from slicing/padding
                    chunks_attention_masks.append(chunk_attention_mask) # These are already on device from slicing/padding

                # Stack operation creates a new tensor, ensure it's on device
                batch_input_ids.append(torch.stack(chunks_input_ids))
                batch_attention_masks.append(torch.stack(chunks_attention_masks))

        # After the loop, ensure all collected items are on device before final stacking
        # This is redundant if all appends were already on device, but acts as a final safeguard.
        final_batch_input_ids = [item.to(self.device) for item in batch_input_ids]
        final_batch_attention_masks = [item.to(self.device) for item in batch_attention_masks]
    ...
    def _tokenize_single_text(self, texts: list[str], max_length: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        attention_mask = encoded['attention_mask'].to(self.device, dtype=torch.long)
    ...
    def _tokenize_multi_tag_text(self, list_of_tag_lists: list[list[str]], max_tags: int, max_tag_length: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for tags_for_single_item in list_of_tag_lists:
            ...
            if not tags_for_single_item:
                ...
                padding_ids = torch.full(pad_shape, self.tokenizer.pad_token_id, dtype=torch.long, device=current_device)
                padding_mask = torch.full(pad_shape, 0, dtype=torch.long, device=current_device)
                ...
                for tag in tags_for_single_item[:max_tags]:
                    ...
                    item_input_ids.append(encoded_tag['input_ids'].squeeze(0).to(current_device, dtype=torch.long))
                    item_attention_masks.append(encoded_tag['attention_mask'].squeeze(0).to(current_device, dtype=torch.long))
            ...
            # This line should now work correctly because all tensors in item_input_ids (and item_attention_masks)
            # are consistently on the same device (either from tokenization or direct creation on device).
            batch_input_ids.append(torch.stack(item_input_ids).to(current_device, dtype=torch.long))
            batch_attention_masks.append(torch.stack(item_attention_masks).to(current_device, dtype=torch.long))

        # Ensure the final stacked output is on the device
        return torch.stack(batch_input_ids).to(current_device, dtype=torch.long), torch.stack(batch_attention_masks).to(current_device, dtype=torch.long)
    ...
    def forward(self, raw_batch_data: dict) -> torch.Tensor|None:
        ...
        # Category should be treated as multi-tag if NavermapReviewDataset defines it as 'list_of_strings'
        category_input_ids, category_attention_mask = self._tokenize_multi_tag_text(
            raw_batch_data['category'], 1, self.aux_text_max_len
        )
        ...
        category_pooled_emb = torch.mean(category_emb, dim=1)
        category_emb_proj = self.proj_category(category_pooled_emb)
        ...
        if self.strategy == "option1_cross_attention_results":
            ...
            attended_visit_keywords_emb = self.attention_visit_keywords(review_emb_proj, visit_keywords_emb_proj.unsqueeze(1)) #
            attended_keyword_tags_hangul_emb = self.attention_keyword_tags_hangul(review_emb_proj, keyword_tags_hangul_emb_proj.unsqueeze(1))
            ...
            fused_text_features = self.proj_default_final(fused_text_features) # Project back to fusion_embed_dim
...
class ImagePreprocessor(torch.nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.no_sample_images_embedding = nn.Parameter(torch.zeros(self.fusion_embed_dim)).to(self.device)

        self.encoder_to_fusion_projection = nn.Linear(self.encoder_output_dim, self.fusion_embed_dim)
        ...
        if self.strategy in ["option1_global_mean_pool", "option3_global_max_pool"]:
            self.final_projection = nn.Linear(self.fusion_embed_dim, self.fusion_embed_dim)
            ...
            self.final_projection = nn.Linear(2 * self.fusion_embed_dim, self.fusion_embed_dim)
        else: # default: option 1, AKA global mean pool
            self.final_projection = nn.Linear(self.fusion_embed_dim, self.fusion_embed_dim)
    ...
    def _process_image_batch_type(self, list_of_links_for_batch: list[list[str]]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        image_presence_masks = torch.zeros((batch_size, self.max_images_per_sample), dtype=torch.long, device=self.device)
        ...
        for i, item_image_links in enumerate(list_of_links_for_batch):
            ...
            for j, link in enumerate(item_image_links[:self.max_images_per_sample]):
                ...
                pixel_values_on_device = processed_input['pixel_values'].squeeze(0).to(self.device)
                current_sample_pixel_values.append(pixel_values_on_device)
            ...
            if num_actual_images < self.max_images_per_sample:
                ...
                for _ in range(self.max_images_per_sample - num_actual_images):
                    in_device_dummy = dummy_pad_image_tensor.to(self.device)
                    current_sample_pixel_values.append(in_device_dummy)
        ...
        batched_pixel_values_for_encoder = torch.stack(all_processed_pixel_values_flat).to(self.device)
    ...
    def _aggregate_global_mean_pool(self, features_batched: torch.Tensor, presence_masks: torch.Tensor, batch_size: int) -> torch.Tensor:
        ...
        aggregated_features_raw = sum_features / (num_real_items + 1e-9)
        # Project the aggregated features to fusion_embed_dim
        aggregated_features_projected = self.encoder_to_fusion_projection(aggregated_features_raw)

        # Use torch.where to substitute the learnable embedding where num_real_items is 0
        final_aggregated_features = torch.where(
            num_real_items == 0,
            self.no_sample_images_embedding.unsqueeze(0).expand(batch_size, -1), # already on device from init
            aggregated_features_projected
        )
    ...
    def _aggregate_global_max_pool(self, features_batched: torch.Tensor, presence_masks: torch.Tensor, batch_size: int) -> torch.Tensor:
        ...
        aggregated_features_raw, _ = masked_features.max(dim=1) # (batch_size, encoder_output_dim)
        # Project the aggregated features to fusion_embed_dim
        aggregated_features_projected = self.encoder_to_fusion_projection(aggregated_features_raw)
        ...
        final_aggregated_features = torch.where(
            ...
            aggregated_features_projected,
            self.no_sample_images_embedding.unsqueeze(0).expand(batch_size, -1) # moved to device from init
        )
        return final_aggregated_features
    ...
    def forward(self, raw_batch_data: dict) -> torch.Tensor:
        ...
        if batch_size == 0:
            return torch.empty(0, self.fusion_embed_dim, device=self.device) # Return empty tensor for empty batch
