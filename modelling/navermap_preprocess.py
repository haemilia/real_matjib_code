...
class TextPreprocessor(torch.nn.Module):
    ...
    def _tokenize_and_chunk_long_text(self, texts: list[str], max_length: int, chunk_overlap: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for text in texts:
            ...
            if input_ids.size(0) <= max_length:
                # Use torch.full_like for padding to create tensors already on device
                ...
                batch_input_ids.append(padded_input_ids.unsqueeze(0)) # .unsqueeze(0) creates new tensor, must be on device
        ...
        # This is where the problematic tensor might be caught if it slipped through previous steps.
        ...
        # Final stack, ensure the result is on device.
        # This should always be true if the list elements are already on device.
    ...
    def _tokenize_multi_tag_text(self, list_of_tag_lists: list[list[str]], max_tags: int, max_tag_length: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        current_device = self.device # Using the consistently named 'device'
        ...
        for tags_for_single_item in list_of_tag_lists:
            ...
            if not tags_for_single_item:
                ...
                # FIX: Create padding tensors DIRECTLY on the target device
                padding_ids = torch.full(pad_shape, self.tokenizer.pad_token_id, dtype=torch.long, device=current_device) # <-- ADDED device=current_device
                padding_mask = torch.full(pad_shape, 0, dtype=torch.long, device=current_device) # <-- ADDED device=current_device
                ...
                for tag in tags_for_single_item[:max_tags]:
                    ...
                    # This part was already correctly moving to device
        ...
        return torch.stack(batch_input_ids).to(current_device, dtype=torch.long), \
               torch.stack(batch_attention_masks).to(current_device, dtype=torch.long)
